...
-- Create policies with unique names and version suffixes
DO $$ 
BEGIN
    -- Select policy
    IF NOT EXISTS (
        ...
        WHERE tablename = 'card_favorites' 
        AND policyname = 'card_favorites_select_policy_v2'
    ) THEN
        CREATE POLICY "card_favorites_select_policy_v2"
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'card_favorites' 
        AND policyname = 'card_favorites_insert_policy_v2'
    ) THEN
        CREATE POLICY "card_favorites_insert_policy_v2"
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'card_favorites' 
        AND policyname = 'card_favorites_delete_policy_v2'
    ) THEN
        CREATE POLICY "card_favorites_delete_policy_v2"
